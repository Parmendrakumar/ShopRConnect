...
using RetailerApp;
using RetailerApp.Droid.Renderers;
...
namespace RetailerApp.Droid.Renderers
{
    class MainPageRenderer : VisualElementRenderer<MainPage>, IOnTabClickListener
    {
        ...
        //    _bottomBar.Measure(MeasureSpec.MakeMeasureSpec(width, MeasureSpecMode.Exactly), MeasureSpec.MakeMeasureSpec(height, MeasureSpecMode.AtMost));
        //    int tabsHeight = Math.Min(height, Math.Max(_bottomBar.MeasuredHeight, _bottomBar.MinimumHeight));

        //    if (width > 0 && height > 0)
        //    {
        //        _currentPage.ContainerArea = new Rectangle(0, 0, context.FromPixels(width), context.FromPixels(_layout.MeasuredHeight));
        //        ObservableCollection<Element> internalChildren = _currentPage.InternalChildren;
        ...
        //        _bottomBar.Measure(MeasureSpec.MakeMeasureSpec(width, MeasureSpecMode.Exactly), MeasureSpec.MakeMeasureSpec(tabsHeight, MeasureSpecMode.Exactly));
